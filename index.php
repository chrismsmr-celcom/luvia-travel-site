...
form div[class*="border-r"],
form div[class*="flex-1"],
.tab-content-desktop,
.p-5,
div[class*="p-3"],
                ...
                <div class="bg-white rounded-2xl rounded-tl-none shadow-2xl overflow-hidden">
                    ...
                    <div id="packages-content-desktop" class="tab-content-desktop hidden p-5">
                        <form action="search.php" method="GET" class="flex flex-row flex-wrap items-stretch gap-0">
                            ...
                            <div>
                                <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-bold px-8 py-3 rounded-xl text-sm uppercase shadow-md h-full" style="margin: 8px;">Rechercher</button>
                            </div>
                        </form>
                    </div>
...
<div class="mobile-search-container" id="mobileSearchContainer">
    ...
    <div class="mobile-content">
        <div class="flex mb-4 mobile-tab-nav">
            <div class="mobile-tab active" data-mobile-tab="flights"><span>Vols</span></div>
            <div class="mobile-tab" data-mobile-tab="hotels"><span>Hôtels</span></div>
            <div class="mobile-tab" data-mobile-tab="cars"><span>Location</span></div>
            <div class="mobile-tab" data-mobile-tab="packages"><span>Packages</span></div>
            <div class="mobile-tab" data-mobile-tab="insurance"><span>Assurance</span></div>
        </div>
        ...
        <div id="mobile-flights" class="mobile-tab-content active">
            <form action="search.php" method="GET">
                ...
                <div class="mobile-field"><input type="text" name="origin" placeholder="Départ" autocomplete="off" class="w-full"></div>
                <div class="mobile-field"><input type="text" name="destination" placeholder="Arrivée" autocomplete="off" class="w-full"></div>
                <div class="mobile-field"><input type="date" name="departure_date" min="<?php echo $today; ?>" class="w-full"></div>
                <div id="mobile-return-date" class="mobile-field"><input type="date" name="return_date" min="<?php echo $today; ?>" class="w-full"></div>
        ...
        <div id="mobile-hotels" class="mobile-tab-content hidden">
            <form action="search.php" method="GET">
                <input type="hidden" name="type" value="hotel">
                <div class="mobile-field"><input type="text" name="city" placeholder="Destination" autocomplete="off" class="w-full" required></div>
                <div class="mobile-field"><input type="date" name="check_in" min="<?php echo $today; ?>" class="w-full" required></div>
                <div class="mobile-field"><input type="date" name="check_out" min="<?php echo $today; ?>" class="w-full" required></div>
                <div class="mobile-field"><input type="number" name="adults" placeholder="Adultes" value="2" min="1" class="w-full"></div>
                <div class="mobile-field"><input type="text" name="children_ages" placeholder="Âges enfants (ex: 5,8,11)" class="w-full"></div>
                <button type="submit" class="search-btn-mobile">RECHERCHER</button>
            </form>
        </div>
        ...
        <div id="mobile-insurance" class="mobile-tab-content hidden">
            <form action="search.php" method="GET">
                ...
                <div class="mobile-field"><input type="number" name="duration" placeholder="Durée (jours)" min="1" max="360" value="7" class="w-full" required></div>
                <div class="mobile-field"><input text="text" name="ages" placeholder="Âges (ex: 30,25,8,5)" class="w-full" required></div>
                <button type="submit" class="search-btn-mobile">RECHERCHER</button>
            </form>
        </div>
    </div>
...
if (tripRadios.length) {
    tripRadios.forEach(r => {
        r.addEventListener('change', function() {
            ...
            if (val === 'oneway') {
                returnDateDivDesktop?.classList.add('hidden');
                roundtripDiv?.classList.remove('hidden');
                multicityDiv?.classList.add('hidden');
            } else if (val === 'multicity') {
                returnDateDivDesktop?.classList.remove('hidden');
                roundtripDiv?.classList.add('hidden');
                multicityDiv?.classList.remove('hidden');
                if (window.multiCount === 0 && typeof addMultiCityDesktop === 'function') addMultiCityDesktop();
            } else {
                returnDateDivDesktop?.classList.remove('hidden');
                roundtripDiv?.classList.remove('hidden');
                multicityDiv?.classList.add('hidden');
...
function addMultiCityDesktop() {
    ...
    if (removeBtn) removeBtn.addEventListener('click', () => { newDiv.remove(); window.multiCount--; });
}
document.getElementById('add-multicity-desktop')?.addEventListener('click', addMultiCityDesktop);
...
if (mobileTabs.length) {
    mobileTabs.forEach(tab => {
        tab.addEventListener('click', () => {
            ...
            document.getElementById(`mobile-${target}`)?.classList.remove('hidden');
        });
    });
}
...
document.getElementById('close-mobile-passenger-modal')?.addEventListener('click', closeMobileModal);
...
document.getElementById('apply-mobile-passenger')?.addEventListener('click', function() {
    const total = window.mAdults + window.mChildren + window.mInfants;
    let text = total === 1 ? '1 passager' : total + ' passagers';
    const classSelect = document.getElementById('m-class');
    if (classSelect) text += ', ' + classSelect.options[classSelect.selectedIndex]?.text || 'Economy';
...
class CityAutocomplete {
    constructor(inputElement, options = {}) {
        ...
        this.options = { minChars: 2, delay: 300, maxResults: 10, ...options };
    ...
    init() {
        ...
        this.resultsContainer.style.cssText = 'position:absolute;top:100%;left:0;right:0;z-index:99999;background:white;border-radius:12px;box-shadow:0 10px 25px -5px rgba(0,0,0,0.15);max-height:300px;overflow-y:auto;display:none;';
        
        let parent = this.input.parentElement;
        while (parent && !parent.classList.contains('relative')) {
            if (parent.tagName === 'DIV' || parent.tagName === 'FORM') parent.style.position = 'relative';
            parent.style.overflow = 'visible';
            parent = parent.parentElement;
        }
        this.input.parentElement.style.position = 'relative';
        this.input.parentElement.style.overflow = 'visible';
    ...
    onInput(event) {
        ...
        if (query.length < this.options.minChars) { this.hideResults(); return; }
    ...
    async search(query) {
        ...
        try {
            const response = await fetch(`includes/config.php?action=autocomplete&query=${encodeURIComponent(query)}`, { signal: this.currentRequest.signal });
            const data = await response.json();
            if (data.success && data.results) this.displayResults(data.results);
            else this.hideResults();
        } catch (error) { if (error.name !== 'AbortError') console.error('Autocomplete error:', error);
    ...
    displayResults(results) {
        ...
        results.slice(0, this.options.maxResults).forEach((result, index) => {
            ...
            item.addEventListener('click', (e) => { e.stopPropagation(); this.selectResult(result); });
            item.addEventListener('mouseenter', () => { this.selectedIndex = index; this.highlightSelected(); });
    ...
    highlightSelected() {
        ...
        if (this.selectedIndex >= 0 && items[this.selectedIndex]) items[this.selectedIndex].scrollIntoView({ block: 'nearest' });
    }
    ...
    onKeyDown(event) {
        ...
        switch (event.key) {
            case 'ArrowDown': event.preventDefault(); this.selectedIndex = Math.min(this.selectedIndex + 1, items.length - 1); this.highlightSelected(); break;
            case 'ArrowUp': event.preventDefault(); this.selectedIndex = Math.max(this.selectedIndex - 1, -1); this.highlightSelected(); break;
            case 'Enter': event.preventDefault(); if (this.selectedIndex >= 0 && items[this.selectedIndex]) this.selectResult({ value: items[this.selectedIndex].dataset.value, city: items[this.selectedIndex].dataset.city, country: items[this.selectedIndex].dataset.country, type: items[this.selectedIndex].dataset.type }); break;
            case 'Escape': this.hideResults(); break;
        }
    }
    
    onBlur() { setTimeout(() => { if (!this.resultsContainer.contains(document.activeElement)) this.hideResults(); }, 200); }
...
document.addEventListener('DOMContentLoaded', function() {
    const cityInputs = ['input[name="origin"]', 'input[name="destination"]', 'input[name="city"]', '#mobile-flights input[name="origin"]', '#mobile-flights input[name="destination"]', '#mobile-hotels input[name="city"]', '#mobile-cars input[name="city"]', '#mobile-packages input[name="destination"]'];
    cityInputs.forEach(selector => {
        document.querySelectorAll(selector).forEach(input => { if (!input._autocomplete) input._autocomplete = new CityAutocomplete(input); });
    });
    new MutationObserver(function(mutations) {
        mutations.forEach(mutation => {
            mutation.addedNodes.forEach(node => {
                if (node.nodeType === 1 && node.querySelectorAll) {
                    node.querySelectorAll('input[name="multi_origin[]"], input[name="multi_destination[]"]').forEach(input => { if (!input._autocomplete) input._autocomplete = new CityAutocomplete(input); });
                }
            });
        });
...
<section class="py-20 bg-white">
    <div class="container mx-auto px-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div class="group relative overflow-hidden rounded-2xl shadow-lg cursor-pointer" onclick="window.location.href='<?php if($destination['type'] == 'hotel') { echo 'search.php?type=hotel&city=' . urlencode($destination['city']); } else { echo 'package-detail.php?id=' . urlencode($destination['id']); } ?>'">
                ...
                <div class="p-5">
                    ...
                    <div class="flex flex-wrap gap-2 mb-4">
                        ...
                        <div class="flex items-center gap-1 bg-gray-100 rounded-full px-3 py-1">
                            <?php if($item == 'flight'): ?>
                            <svg class="w-3 h-3 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M12 19l9 2-9-18-9 18 9-2z"></path></svg><span class="text-xs text-gray-700">Vol</span>
                            <?php elseif($item == 'hotel'): ?>
                            <svg class="w-3 h-3 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path></svg><span class="text-xs text-gray-700">Hôtel</span>
                            <?php elseif($item == 'car'): ?>
                            <svg class="w-3 h-3 text-purple-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M8 7h12m0 0l-4-4m4 4l-4 4m-12 4H4a2 2 0 01-2-2V5a2 2 0 012-2h12a2 2 0 012 2v4m-8 4h12m0 0l-4 4m4-4l-4-4"></path></svg><span class="text-xs text-gray-700">Voiture</span>
                            <?php elseif($item == 'activities'): ?>
                            <svg class="w-3 h-3 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"></path></svg><span class="text-xs text-gray-700">Activités</span>
                            <?php endif; ?>
                        </div>
                        <?php endforeach; ?>
                    </div>
                    <div class="flex items-center justify-between pt-3 border-t border-gray-100">
                        <div><span class="text-2xl font-bold text-blue-600"><?php echo displayPrice($package['price']); ?></span><span class="text-gray-400 text-xs">/pers</span></div>
                        <button class="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2 rounded-full text-sm font-semibold transition flex items-center gap-2 shadow-md">Voir détail<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M9 5l7 7-7 7"></path></svg></button>
                    </div>
                </div>
            </div>
            <?php endforeach; ?>
